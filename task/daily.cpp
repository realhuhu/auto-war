...
void hero_center() {
    ...
    run_until.emplace_back(std::make_unique<UntilImage>("/英雄中心/英雄中心.png"));
    ...
    if (config["免费抽奖"]) {
        while (!state.stopFlag.load()) {
            ...
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/英雄中心/英雄抽奖.png", "top_center"));
            clicker = clicker->locate(start_until, run_until);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilIfImage>("/英雄中心/确定.png"));
            clicker = clicker->click(start_until, click_until, run_until, similarity_selector, 0, 1);

            if (clicker->founded()) {
                clear_until(start_until, click_until, run_until);
                run_until.emplace_back(std::make_unique<UntilImage>("/英雄中心/确定.png", "inner", true));
    ...
    if (config["英雄训练"]) {
        ...
        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/英雄中心/英雄列表.png"));
        clicker->click(start_until, click_until, run_until);
        ...
        while (!state.stopFlag.load()) {
            ...
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/英雄中心/开始训练.png"));
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/英雄中心/关闭窗口.png", "inner", true));
    clicker->click(start_until, click_until, run_until);
}
...
void war_center() {
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/战争学院/战争学院标题.png"));
    clicker->clickIfFound(start_until, click_until, run_until);
    ...
    if (config["技能训练"]) {
        ...
        clear_until(start_until, click_until, run_until);
        click_until.emplace_back(std::make_unique<UntilImage>("/战争学院/次数不足.png"));
        clicker = clicker->click(start_until, click_until, run_until, random_selector, 0.5);

        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/战争学院/关闭窗口.png"));
        clicker = clicker->locate(start_until, run_until, position_selector("x_center", "min"));

        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/战争学院/关闭窗口.png", "inner", true));
        clicker->click(start_until, click_until, run_until);
    }
    ...
    if (config["军事演习"]) {
        ...
        if (clicker->templatePath == "/战争学院/空演习场.png") {
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/战争学院/占领演习场.png"));
            clicker = clicker->click(start_until, click_until, run_until);
            ...
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/战争学院/演习中.png"));
            clicker->click(start_until, click_until, run_until);
        }
    }
...
void country_chest() {
    ...
    clear_until(start_until, click_until, run_until);
    click_until.emplace_back(std::make_unique<UntilImage>("/国家宝箱/国家宝箱图标.png"));
    clicker = clicker->clickIfFound(start_until, click_until, run_until);

    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/国家宝箱/国家宝箱标题.png"));
    clicker->clickIfFound(start_until, click_until, run_until);
    ...
    if (config["领取战功奖励"]) {
        while (!state.stopFlag.load()) {
            clicker = std::make_unique<ImageClicker>("/国家宝箱/战功进度宝箱.png", 0.5, 0.9, 60, "rgb");

            if (!clicker->founded())break;

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/国家宝箱/战功进度宝箱.png", "inner", true));
            clicker->click(start_until, click_until, run_until);
        }
    }
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/国家宝箱/关闭窗口.png", "inner", true));
    run_until.emplace_back(std::make_unique<UntilImage>("/国家宝箱/关闭窗口.png"));
    clicker = clicker->click(start_until, click_until, run_until);
...
void guild() { //TODO
    ...
    if (config["领取公会战奖励"]) {
        ...
        if (clicker->founded()) {
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/领取奖励.png"));
            clicker = clicker->click(start_until, click_until, run_until);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/领取奖励.png", "right"));
            clicker = clicker->click(start_until, click_until, run_until);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/确定领取公会战奖励.png"));
            clicker = clicker->click(start_until, click_until, run_until);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/报名公会战.png"));
            clicker = clicker->click(start_until, click_until, run_until);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/确定报名.png"));
            clicker = clicker->click(start_until, click_until, run_until);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/关闭公会战窗口.png"));
            clicker = clicker->click(start_until, click_until, run_until);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/公会.png"));
            clicker->click(start_until, click_until, run_until);
        }
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/公会福利.png"));
    clicker->click(start_until, click_until, run_until);
    ...
    if (config["领取活跃度奖励"]) {
        ...
        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/关闭公会捐献窗口.png", "inner", true));
        clicker->click(start_until, click_until, run_until);
    }
...
//        }
    ...
    if (config["领取公会战役宝箱"]) {
        ...
        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/查看成员排名.png"));
        run_until.emplace_back(std::make_unique<UntilIfImage>("/公会领奖/领取公会战役奖励.png"));
        clicker = clicker->click(start_until, click_until, run_until);

        if (clicker->founded()) {
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/领取公会战役奖励.png", "inner", true));
            clicker->click(start_until, click_until, run_until);
        }
    }
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/公会领奖/关闭公会争霸窗口.png", "inner", true));
    clicker->click(start_until, click_until, run_until);
}
...
void admiral() {
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/将领抽奖/将领面板.png"));
    clicker->clickIfFound(start_until, click_until, run_until);
    ...
    if (config["免费抽奖"]) {
        while (!state.stopFlag.load()) {
            ...
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/将领抽奖/将领抽奖.png", "top_center"));
            clicker = clicker->locate(start_until, run_until);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/将领抽奖/确定.png"));
            clicker = clicker->click(start_until, click_until, run_until, similarity_selector, 0, 1);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/将领抽奖/确定.png", "inner", true));
            clicker->click(start_until, click_until, run_until);
        }
    }
    ...
    if (config["免费学习书"]) {
        ...
        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/将领抽奖/免费收集.png"));
        clicker = clicker->click(start_until, click_until, run_until);

        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilIfImage>("/将领抽奖/次数已用尽.png"));
        clicker = clicker->click(start_until, click_until, run_until, similarity_selector, 0, 1);

        if (clicker->founded()) {
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/将领抽奖/关闭窗口.png"));
            clicker = clicker->locate(start_until, run_until, position_selector("x_center", "min"));

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/将领抽奖/关闭窗口.png", "inner", true));
...
void adviser() {
    ...
    if (config["免费抽奖"]) {
        while (!state.stopFlag.load()) {
            ...
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/参谋抽奖/参谋抽奖.png", "top_center"));
            clicker = clicker->locate(start_until, run_until);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/参谋抽奖/确定.png"));
            clicker = clicker->click(start_until, click_until, run_until, similarity_selector, 0, 1);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/参谋抽奖/确定.png", "inner", true));
            clicker->click(start_until, click_until, run_until);
        }
    ...
    if (config["免费技能书"]) {
        ...
        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/参谋抽奖/参谋列表.png"));
        run_until.emplace_back(std::make_unique<UntilIfImage>("/参谋抽奖/免费收集.png", "none", false, "rgb"));
        clicker = clicker->click(start_until, click_until, run_until);

        if (clicker->founded()) {
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/参谋抽奖/免费收集.png", "none", true, "rgb"));
            clicker->click(start_until, click_until, run_until);
        }
    }
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/参谋抽奖/关闭窗口.png", "inner", true));
    clicker->click(start_until, click_until, run_until);
}
...
void mortar() {
    ...
    for (const auto &i: std::vector<std::string>{"绿"}) { //TODO 蓝、紫色火炮派遣
        while (!state.stopFlag.load()) {
            ...
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/火炮抽奖/派遣.png", "top_center"));
            clicker = clicker->locate(start_until, run_until);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/火炮抽奖/确定.png"));
            clicker = clicker->click(start_until, click_until, run_until, similarity_selector, 0, 1);

            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/火炮抽奖/确定.png", "inner", true));
            clicker->click(start_until, click_until, run_until, similarity_selector, 0, 1);
        }
    }
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/火炮抽奖/关闭窗口.png", "inner", true));
    clicker->click(start_until, click_until, run_until);
...
void equipment() {
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/配件抽奖/配件中心标题.png"));
    clicker->click(start_until, click_until, run_until, similarity_selector);

    clicker = std::make_unique<ImageClicker>("/配件抽奖/野外.png");

    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/配件抽奖/野外场景.png"));
    run_until.emplace_back(std::make_unique<UntilImage>("/配件抽奖/免费探索.png"));
    clicker = clicker->click(start_until, click_until, run_until);


    clear_until(start_until, click_until, run_until);
    click_until.emplace_back(std::make_unique<UntilImage>("/配件抽奖/次数用尽.png"));
    clicker = clicker->click(start_until, click_until, run_until, similarity_selector, 0, 1);

    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/配件抽奖/次数用尽.png", "inner", "true"));
    clicker->click(start_until, click_until, run_until);
    ...
    for (const auto &i: std::vector<std::string>{"工厂", "实验室"}) {
        ...
        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilIfImage>("/配件抽奖/免费探索次数.png"));
        clicker = clicker->click(start_until, click_until, run_until, similarity_selector, 0, 1);

        if (!clicker->founded())continue;

        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/配件抽奖/免费探索.png", "top"));
        clicker = clicker->locate(start_until, run_until);

        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/配件抽奖/免费探索.png", "inner", "true"));
        clicker->click(start_until, click_until, run_until, similarity_selector, 0, 1);
    }
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/配件抽奖/关闭窗口.png", "inner", true));
    clicker->click(start_until, click_until, run_until);
}
...
void arms() {
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/军备抽奖/军备研究提示.png"));
    clicker->click(start_until, click_until, run_until, similarity_selector);
    ...
    for (const auto &i: std::vector<std::string>{"机械", "自动化"}) {
        ...
        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/军备抽奖/" + i + "制造厂场景.png"));
        ...
        while (!state.stopFlag.load()) {
            ...
            clear_until(start_until, click_until, run_until);
            run_until.emplace_back(std::make_unique<UntilImage>("/军备抽奖/免费制造.png", "top_center"));
            clicker = clicker->locate(start_until, run_until);
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/军备抽奖/关闭窗口.png", "inner", true));
    clicker->click(start_until, click_until, run_until);
}
...
void guild_building() {
    ...
    if (!clicker->founded()) {
        ...
        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/公会建筑/场景缩放.png"));
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/公会建筑/打开公会建筑.png"));
    clicker = clicker->click(start_until, click_until, run_until);

    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/公会建筑/公会建筑标题.png"));
    clicker = clicker->click(start_until, click_until, run_until);

    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/公会建筑/团体任务.png"));
    clicker = clicker->click(start_until, click_until, run_until);

    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/公会建筑/参与任务.png"));
    clicker = clicker->click(start_until, click_until, run_until);

    clicker->click();

    auto iterator = CV::find_positions(CV::get_screen(), "/公会建筑/需要人数.png");
    ...
    while (it != iterator.end() and !state.stopFlag.load()) {
        ...
        clicker = std::make_unique<ImageClicker>("/公会建筑/领取奖励.png", 0, 0.9, 60, "rgb");

        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/公会建筑/确定领取.png"));
        clicker = clicker->clickIfFound(start_until, click_until, run_until);

        clear_until(start_until, click_until, run_until);
        run_until.emplace_back(std::make_unique<UntilImage>("/公会建筑/确定领取.png", "inner", true));
        clicker->clickIfFound(start_until, click_until, run_until);
    ...
    clear_until(start_until, click_until, run_until);
    run_until.emplace_back(std::make_unique<UntilImage>("/公会建筑/关闭窗口.png", "inner", "true"));
    clicker->click(start_until, click_until, run_until);
}
