...
ScreenshotArea::~ScreenshotArea() { refreshTimer->stop(); }
...
void ReplaceDialog::openFile() {
    ...
    if (!filePath.startsWith(resDir)) {
        tipLabel->setText("只能替换 /游戏图片 文件夹下的图片");
        return;
    }
    ...
    if (!pixmap.isNull()) selectedImageLabel->setPixmap(pixmap.scaled(selectedImageLabel->size(), Qt::KeepAspectRatio));
}
...
void ReplaceDialog::showScreenshot(const QPoint p1, const QPoint p2) {
    ...
    auto screen = CV::getScreen(hwnd, Mode::RGB);
...
void ReplaceDialog::closeEvent(QCloseEvent *) { parentWidget()->showNormal(); }
